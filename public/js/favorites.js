fetch("/public/mock/suggestions.json")
    ...
    .then(data => {
        fetch("/public/mock/components/tea-card.html")
    ...
    fetch("/public/mock/favorites.json")
    .then(response => response.json())
    .then(data => {
        fetch("/public/html/components/tea-card.html")
            ...
            .then(teaCardData => {
                var parser = new DOMParser();
                var html = parser.parseFromString(teaCardData, 'text/html');
                let favoritesContainer = document.getElementById("favorites__cards");
                let teaCard = html.querySelector(".tea-card");
                let teaCards = [];
                for (const element of data) {
                    let teaCardTmp = teaCard?.cloneNode(true);
                ...
                teaCards.forEach(elem => favoritesContainer?.appendChild(elem));
            })
    });